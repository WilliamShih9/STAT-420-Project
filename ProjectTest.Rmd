...
```{r, message = FALSE}
bike_tst$Predicted_BikeCount =  predict(bic_sqrt, bike_tst)^2
hour_holidayerror = bike_tst %>%
...
plot_hour_holidayerror = ggplot(hour_holidayerror, aes(x = Hour, group = Holiday, linetype = Holiday)) + 
  ...
  ggtitle("Mean Actual vs Predicted Hour of Day and Holiday or No Holiday (Test Set)")
```
```{r, message = FALSE}
hour_seasonerror = bike_tst %>%
...
plot_hour_seasonerror = ggplot(hour_seasonerror, aes(x = Hour, group = Season)) + theme_bw() +
  ...
  ggtitle("Mean Actual vs Predicted Hour of Day and Season (Test Set)")
