...
evaluate = function(models){
  ...
  R2test_list = prettyNum(sapply(models, get_R2), digits = 3, format = "f")
  ...
  frame[,-1] = formatC(frame[,-1], digits = 1, format = "f", big.mark = ",")
  frame = cbind(frame, R2adj_list, R2test_list)
...
kable(frame, digits = 2, caption = "Evaluation of Models (Test RMSE of XBoost Tree = 183.80)") %>%
...
```{r, warning = FALSE}
frame2 = evaluate(models)
rownames(frame2) = c("All Interaction", "AIC Model", "BIC Model")
kable(frame2, digits = 2, caption = "Evaluation of Models (Test RMSE of XBoost Tree = 183.80)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
```{r, message = FALSE}
...
transformation_power = function(pow){
  ...
  n = length(pred_test)
  c(Adj_R2 = round(cor(pred_train, bike_trn$RentedBikeCount)^2*R2_factor, 3), 
         Test_RMSE = round(sqrt(1/n * sum((actual_test-pred_test)^2)), 1))
}
...
kable(power_RMSE, caption = "Adjusted R^2 and Test RMSE using power transformations of response variable of BIC model (Predictions Clamped at 0)") %>%
...
frame3 = rbind(frame2, c(99, "N/A","N/A", formatC(bic_sqrt_rmse, digits = 1, format = "f"), 
                       formatC(bic_sqrt_adj_R2, digits = 3),
                       formatC(bic_test_R2, digits = 3)))
rownames(frame3) = c("All Interaction", "AIC Model", "BIC Model","BIC Model with Sqrt Transform") 
kable(frame3, caption = "Evaluation of Models Again") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
