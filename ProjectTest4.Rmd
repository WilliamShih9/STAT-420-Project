...
power_RMSE = t(sapply(seq(0.2,1,0.1), transformation_power, form = bic_formula, offset = 0))
colnames(power_RMSE) = c("Adjusted R^2","Test RMSE","Proportion of Predictions < 0 in Test Set")
rownames(power_RMSE) = try_powers
table_transform = kable(power_RMSE, digits = c(3,1,4), caption = "Adjusted R^2 and Test RMSE using power transformations of response variable of BIC model (Predictions Clamped at 0)") %>%
...
```{r, warning = FALSE}
Predicted = predict(best_model, data)
Predicted2 = ifelse(Predicted > 0, Predicted^2, -Predicted^2)
Predicted3 = Predicted^2
