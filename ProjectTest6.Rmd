...
data$Weekend = ifelse(Week == "Saturday" | Week == "Sunday", 1, 0)
...
Weekend is added as a new variable, derived from Date. The Weekend variable is set to true if the day of the week is Saturday or Sunday and set to false otherwise.
...
correlations = round(cor(data %>% select(-c(Day, Weekend, Month, Season, Holiday, Functional))), 2)
...
#best = step(bic_sqrt_full, span = RentedBikeCount ~ 1, direction = "backward", k = log(trn_size), trace = 0)
...
plot_CV = ggplot(res, aes(x = MAE, y = MSE,  color = model)) +
  geom_point(size = 3) + theme_bw() + xlab("Mean Absolute Error") +
