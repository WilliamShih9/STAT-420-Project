...
plot_diag2 = plot_diagnostics(best_model, "Selected Model with Sqrt Transform")
```
...
From the Table 3.2 below it is evident that the square root transformation performed the best on the test set with adjusted $R^{2}$ of 0.902 and RMSE of 211.3.
...
Table 3.3 below shows the error metrics on the Test set for different models. The square root transform model with manually selection, lets call this the Selected Model (Sqrt Transform), of predictors emerged as the best model with the least error LOOCV-RMSE and least BIC

```{r tab3, echo = FALSE, fig.align = 'center'}
table_eval2
```

The Fitted Vs Residuals plot in Figure \@ref(fig:fig3) for the selected model with square root transformation of the response variable shows close to constant variance compared to the model without transforming the response variable and there is no great improvement in the Q-Q plot as it still shows fat tails
