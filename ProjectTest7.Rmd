...
We began with several initial models - an multiple linear regressions (MLR) additive model and several interaction models, ending will a "full" interaction model with all two-way interaction between the predictors.
...
aic_formula = formula(RentedBikeCount ~ . + factor(Hour)*Weekend + I(Temperature^2) + I(Wind^2) +
  Month:(Temperature+DewPoint+SolarRadiation+Rainfall+Season+Functional)+
  Hour:(Temperature+Humidity+Wind+Visibility+DewPoint+SolarRadiation+Rainfall+Snowfall+Season+Holiday+Functional)+
  Temperature:(Wind+Visibility+DewPoint+Rainfall+Season+Holiday+Weekend)+
  Humidity:(Wind+Visibility+DewPoint+SolarRadiation+Rainfall+Season+Functional+Weekend)+
  Wind:(Visibility+DewPoint + SolarRadiation + Season + Holiday + Weekend) + Visibility:(DewPoint + SolarRadiation) +
  DewPoint:(SolarRadiation + Rainfall + Season + Holiday + Functional + Weekend) +
  SolarRadiation:(Rainfall + Season + Holiday + Functional + Weekend) +
  Rainfall:(Snowfall + Season + Holiday + Weekend) +
  Season:(Holiday + Functional) + Holiday:Functional + Functional:Weekend +
  I(Temperature^2):(Month + DewPoint + Rainfall + Season + Holiday) +
  I(Wind^2):(Hour+Wind+Visibility+Rainfall+Snowfall+Season +Holiday+ Weekend))
bic_formula = formula(RentedBikeCount ~ Month+Hour+Temperature+Humidity+Wind+Visibility+DewPoint+SolarRadiation + 
  Rainfall + Snowfall + Season + Holiday + Functional + Weekend + factor(Hour)*Weekend + I(Temperature^2) + I(Wind^2) +
  Month:(Hour+Temperature+SolarRadiation+Functional+Season)+
  Hour:(Temperature+Humidity+Wind+DewPoint+SolarRadiation+Rainfall+Holiday+Functional)+
  Temperature:(Wind+Visibility+DewPoint+Rainfall+Season+Weekend)+
  Humidity:(SolarRadiation+Functional+Wind+Visibility+DewPoint+Season+Weekend)+
  Wind:(Visibility+DewPoint + SolarRadiation + Rainfall +Season) +
  Visibility:DewPoint + DewPoint:(SolarRadiation + Rainfall + Season + Functional + Weekend) +
  SolarRadiation:(Season + Functional + Weekend) + Rainfall:Snowfall + Season:(Functional+Holiday) + Holiday:Functional + 
  I(Temperature^2):(Month + DewPoint + Rainfall + Season + Holiday) + I(Wind^2):(Hour+Visibility+Season))
aic_model = lm(aic_formula, data = bike_trn)
bic_model = lm(bic_formula, data = bike_trn)
```

The BIC model was only slightly worse than the AIC model while lowering the number of parameters from 214 to 137 so the model selected from backward stepwise regression using BIC was the initial choice of model.
...
best_formula = formula(sqrt(RentedBikeCount) ~ .-Hour + factor(Hour)*Weekend + I(Temperature^2) + I(Humidity^2) + I(Wind^2) +
...
```{r, warning = FALSE}
#Plot timeline of actual bikes rented vs predicted
...
```{r, warning = FALSE, message = FALSE}
#Plot the actual bike count vs predicted and error in predictions
...
```{r, message = FALSE}
#Plot prediction with holiday analysis
...
```{r, message = FALSE}
#Plot prediction with season analysis
...
#Histogram of symmetric mean absolute percentage error for hours
hourerror = data %>%
  group_by(Hour) %>%
...
```{r, message = FALSE, warning = FALSE}
#Histogram of symmetric mean absolute percentage error for months
montherror = data %>%
  group_by(Month) %>%
  summarize(Error = 100 * Metrics::smape(RentedBikeCount, Predicted_BikeCount))
...
Table 3.1 below shows how the initial models performed on the Test set. The larger models will almost always have a larger R2 value and this is evident from the table. The 'All Interaction with Quadratics' model with 310 coefficients has the highest adjusted R2. We chose the 'All Interaction (Quadratic for Wind/Temp)' model with 214 parameters as the best initial model as it is a less complex model than the largest model and it performed well on all the evaluation metrics on the Test set.
...
From the Table 3.2 below it is evident that the square root transformation performed the best on the test set with adjusted R2 of 0.902.

```{r tab2, echo = FALSE, fig.align = 'center'}
table_transform
```

The Q-Q plot and Fitted Vs residual plot clearly showed the normalcy and equal variance violations. So, the idea is to transform the response variable, so that the regression residuals become normally-distributed. The optimal lambda value from the Box-Cox power transform is 0.3 but we achieved better RMSE and better adjusted R2 in terms of predictions on the Test set with a lambda = 0.5 which is a square root transformation. Also, since the response variable(Rented Bike Count) is a positive number, we do not have the issue of NaN's popping due to a negative number raised to 0.5
```{r fig2, fig.cap = "Box-Cox Plot from BIC Model with no Transform", echo = FALSE, fig.align = 'center'}
plot_boxcox
```

The Fitted Vs Residuals plot in Figure \@ref(fig:fig3)for the best BIC model with square root transform still does not show equal variance and there is no great improvement in the Q-Q plot as it still shows fat tails.
...
Figure \@ref(fig:fig4) and Figure \@ref(fig:fig5) show the MAE and RMSE values of the 10 fold Cross Validation of the selected models. It is evident that the Selected Model with square root transform performed the best with the least MAE and RMSE errors.
...
Table 3.4 shows the evaluation metrics on Test set for various models. Clearly, the Selected Model with square root transform performs the best in terms of all the error metrics chosen.
...
Using this interaction model with wind/temp quadratics as a new base for the "best so far" model, we began to refine this model using AIC and BIC procedures. Table 3.3 shows the results of this exercise. The BIC model performed only slightly worse than the AIC model, but with a substantial decrease in coefficient complexity. For this reason, we chose the BIC model for further testing. Figure \@ref(fig:fig1) illustrates the results of performing a "Fitted vs Residuals" and "Normal Q-Q" plot for the BIC model. As evidenced by this analysis, the constant variance assumption is violated in the model and the normality is also suspect, given the fat tails seen in the Q-Q plot. To alleviate these issues, we analyzed power transformations on the response variable, as shown in Table 3.2. We experimented on transforming the response variable in an attempt to make the regression residuals normally-distributed. The optimal lambda value from the Box-Cox power transform is 0.3 but we achieved better RMSE and better adjusted R2 in terms of predictions on the Test set with a lambda = 0.5 which is a square root transformation. Also, since the response variable (Rented Bike Count) is a positive number, we do not have the issue of NaN's popping due to a negative number raised to 0.5. Based on these results, a square root transformation was preferred based on the Adjusted $R^2$ and Test RMSE values. As mentioned earlier, a log transformation was not appropriate given the amount of zero values in the data. Figure \@ref(fig:fig2) illustrates the effects on the "Fitted vs Residuals" and "Normal Q-Q" after the square root transformation on the response. Although only the Fitted vs Residuals plot improved, the same issues still persist for the equal variance and normality of the model.

Using the 'Full Model', we used a stepwise BIC procedure and manually removed several terms through trial and error to arrive at our best model.  We found the 'Selected Model (Sqrt Transformation)' that, although increased coefficient complexity, provided the best results for the LOOCV-RMSE, BIC, Test RMSE and Test $R^2$.

Once we found our chosen model, we ran a 10-fold cross validation analysis on it, as well as other tested models for comparison.  Figure \@ref(fig:fig3) and \@ref(fig:fig4) show the Selected Model on average performs much better than any other in terms of RMSE and MAE errors. We also looked at our predicated rented bike count vs actual rented bike count from the test data in Figure \@ref(fig:fig5), perfect prediction assuming y=x relationship. While clearly not perfect, this figure shows the predictions follow a similar pattern of the regression line, with predictions starting to overestimate when the bike count grows larger (1500+). This error is also evidenced in Figure \@ref(fig:fig6). Although most points center around zero, particularly at the lower bike rental rates, the model begins to over predict rentals once we have less data at the higher bike rental points.

In Figures \@ref(fig:fig7) and \@ref(fig:fig8), we selected two timelines of 50 consecutive hours to test the predicted bike rentals vs actual rentals. It can be seen that the prediction generally follows the actual trend of rented bikes, though does not match exactly. Figure \@ref(fig:fig9) shows the mean actual vs predicted bike rentals by hour of the day with a comparison to holidays. We can see there is a bias in the model, as there is a spike in predictions that coincides with the jump in holiday actuals and predictions. On average, the prediction is quite close to the actual mean observations, with the exception of the holiday issues stated previously. Figure \@ref(fig:fig10) does the same analysis for season (rather than holiday).  This analysis also shows the average prediction by hour follows the actual observations quite closely.  Figure \@ref(fig:fig12) and \@ref(fig:fig13) also plots our symmetric mean error by hour and month.  We can see that the error slowly rises in the early hours of the day, peaking around the 4 hour mark. September has the highest error of any give month, while the summer months tend to have less errors.

Based on our results discussed above, the model we've chosen seem to work somewhat well to predict the number of bikes rented at any given hour on any given day. Figures \@ref(fig:fig7) and \@ref(fig:fig8) show that, although the average trend is accurate, the number of bikes rented at any given hour can be quite off based on what was predicted.  On the other hand, this model has success in predicting average bikes rented by the hour when look at seasons and holidays (Figures \@ref(fig:fig9) and \@ref(fig:fig10)).  This would help cities and/or companies determine the seasonality of average bike usage throughout the day and predict on average how many bikes should be on hand.
