...
p {  margin-top: 1.3em; margin-bottom: 0; }
</style>
...
The data set contains bike sharing data in Seoul from 12/1/2017 to 11/30/2018. Each observation represents an hour of time. The variable of interest for prediction is the rented bike count for the hour. The 13 variables that will be considered as predictors include the day, the hour of day, temperature, humidity, wind speed, visibility, dew point, solar radiation, rainfall, snowfall, season of year, holiday (boolean), and functioning (boolean). The dataset is associated with two papers published in a journal in February and March 2020 related to gradient tree boosting, but here we'll be only using linear regression. The source of the raw dataset is found at the Seoul government website http://data.seoul.go.kr/

A further description of the dataset can be found at https://archive.ics.uci.edu/ml/datasets/Seoul+Bike+Sharing+Demand

# Methods

We began by importing and cleaning the raw data from the dataset described above.  As part of this, we transformed several predictors into Factor and Integer variables.  We also broke out the 'Date' field into 'Day', 'Month' and 'Year'.  We discarded the 'Year' predictor as the dataset spans only a single year, so it would not be explain anyting in the model. Also, we later discard the `Date` predictor as it has no reason to show any relationship to the number of bikes rented. We introduced a new variable `Week`, which is obtained by finding the day of werk from each day, month, and year combination.
...
day = substr(data$Date,1,2)
month = substr(data$Date,4,5)
Week = paste0(substr(data$Date, 7, 10), "-",month,"-",day)
...
Figure \@ref(fig:fig-2) shows no correlation between day and number of rented bikes as expected. The relationship between the hour of day and the number of rented bikes is a highly complex funtion, so we'll want to consider the hour of day as a categorical variable rather than a continuous one. 
...
#Remove 'Day' as the plot shows it won't a useful predictor
data = dplyr::select(data, -Day)
```

```{r fig-3, message = FALSE, warning = FALSE, fig.cap = "Number of Rented Bikes by Temperature/Humidity/Wind/Visibility", fig.align = 'center'}
...
```{r fig-6, message = FALSE, warning = FALSE, fig.cap = "Number of Rented Bikes by Hour of Day and Day of Week", fig.align = 'center'}
bikeperweekdayhour = data.frame(Bike = data$RentedBikeCount, Hour = data$Hour, Weekday = Week) %>%
  group_by(Hour, Weekday) %>%
  summarize(Count = mean(Bike))
ggplot(bikeperweekdayhour, aes(x = Hour, y = Count, group = Weekday, color = Weekday)) + 
  ylab("Number of Rented Bikes") + geom_line(size = 1) + geom_point() + theme_bw() + 
  ggtitle("Number of Rented Bikes by Hour of Day and Day of Week")
# Add Weekend as a new variable as number of rented bikes varies depending on whether it is Saturday/Sunday vs the other 5 days
data$Weekend = as.factor(ifelse(Week == "Saturday" | Week == "Sunday", 1, 0))
```

Figure \@ref(fig:fig-6) shows that the `Week` variable can be condensed into a single `Weekend` variable since Monday, Tuesday, Wednesday, Thursday, and Friday appear to be identical to each other when considering the pattern of the number of rented bikes by hour. We do not consider removing outliers from this data set as we do not believe that there were measurement or data entry errors made in this data set, so no data will be removed.


 The `get_R2_helper` function for getting the test $R^{2}$ is based on sklearn.metrics.r2_score. https://scikit-learn.org/stable/modules/model_evaluation.html#r2-score
...
get_R2_adj = function(model, power){
  ...
  get_R2_helper(y = bike_trn$RentedBikeCount, yhat = predict(model, bike_trn)^power)*factor
}
...
#Function to evaluate the list of models with metrics chosen with given power transformation to y
...
# Function to find Model Metrics in original data units given a power transformation to y
...
We then split our data into 'Train' and 'Test' data sets.  We used a 70%/30% split of Train/Test, where we will build our model on the 'Train' data and test its performance on the 'Test' data set.
...
We began with several initial models - an multiple linear regressions (MLR) additive model and several interaction models, ending with a "full" interaction model with all two-way interaction between the predictors and quadratics of the predictors.

```{r, message = FALSE, warning = FALSE, fig.height= 15, fig.width = 15}
#Initial model starting points
model_add = lm(RentedBikeCount ~ ., data = bike_trn)
model_fact0 = lm(RentedBikeCount ~ . + factor(Hour), data = bike_trn)
model_fact1 = lm(RentedBikeCount ~ . + factor(Hour)*Weekend, data = bike_trn)
model_fact2 = lm(RentedBikeCount ~ . + factor(Hour)*Weekend + (Temperature * .), data = bike_trn)
model_fact3 = lm(RentedBikeCount ~ . + factor(Hour)*Weekend + (Temperature * .) + (Hour*.), data = bike_trn)
model_fact4 = lm(RentedBikeCount ~ . + factor(Hour)*Weekend + (Temperature * .) + (Humidity*.) + (Hour*.), data = bike_trn)
model_fact5 = lm(RentedBikeCount ~ (.)^2 + factor(Hour)*Weekend, data = bike_trn)
continuous = c("Hour","Temperature","Humidity","Wind","Visibility","DewPoint","SolarRadiation","Rainfall","Snowfall")
full_formula = as.formula(
    paste0('RentedBikeCount ~ (.)^2 + factor(Hour)*Weekend + ', paste0('I(',continuous,'^2)*.',collapse = ' + ')))
model_fact6 = lm(RentedBikeCount ~ (.)^2 + factor(Hour)*Weekend + I(Temperature^2)*. + I(Wind^2)*., data = bike_trn)
model_full = lm(full_formula, data = bike_trn)
models = list(model_add, model_fact0, model_fact1, model_fact2, model_fact3, model_fact4, model_fact5, model_fact6, model_full)
```

```{r, warning = FALSE}
frame = evaluate(models, row_names = c("Additive","Hour as Factor","Hour*Weekend as Factor", "Interaction with Temp",
...
The BIC model performed just as well as the AIC model while lowering the number of parameters from 161 to 137 so the model selected from backward stepwise regression using BIC was the initial choice of model.
...
The `bic_sqrt` model has the same exact parameters as the model selected from backward step wise regression using BIC of `model_fact6`, which is `bic_model`.
The model we chose as the "final model" is the `selected_model` below. `selected_model` was found after first performing backward BIC step wise regression on the `bic_sqrt_full` model (takes a long time to compute) and then manually adjusting the model afterwards. This performed better than the `bic_sqrt` model as the later plots show. The formula for the selected model is found below as `selected_model_formula`.

```{r, warning = FALSE, message = FALSE}
# Code for Table 3.3 and Figure 3.3
bic_sqrt_full = lm(sqrt(RentedBikeCount) ~ (.)^2 + factor(Hour)*Weekend + 
...
selected_model_formula = formula(sqrt(RentedBikeCount) ~ .-Hour + factor(Hour)*Weekend + I(Temperature^2) + I(Humidity^2) + I(Wind^2) +
  I(Visibility^2) + I(DewPoint^2) + I(SolarRadiation^2) + I(Rainfall^2) + I(Snowfall^2) + Month:(Season + Holiday) +
  ...
  Wind:(Visibility + DewPoint) + Visibility:Rainfall + DewPoint:(Rainfall + Season + Functional) + SolarRadiation:Holiday +
  Rainfall:Functional + Season:Holiday +
...
selected_model = lm(selected_model_formula, data = bike_trn)
sqrt_models = list(bic_sqrt, selected_model, bic_sqrt_full)
...
rownames(frame3) = c("All Interaction (Quadratic for Wind/Temp)(Linear)", "AIC Model (Linear)", "BIC Model (Linear)","BIC Model (Sqrt Transform)", "Selected Model (Sqrt Transform)", "Full Model (Sqrt Transform)") 
table_eval2 = get_table(frame3, title = "Evaluation of Models (LOOCV-RMSE/BIC of Transformed vs Non-Transformed Not Comparable)")
plot_diag2 = plot_diagnostics(selected_model, "Selected Model with Sqrt Transform")
```

We utilized 10-fold cross-validation to measure how much better the selected model is compared to the model chosen by BIC and the naive additive model.

```{r, warning = FALSE, message = FALSE}
# Code for Figures 3.4 and 3.5
library(DAAG)
library(Metrics)
kfold = 10
cv_evaluate = list(model_add, bic_model, bic_sqrt, selected_model)
# Calculate the CV with seed = 40 for all 4 models and return actual,predicted,cvpredicted, and fold number
cv_bic = lapply(cv_evaluate,function(model){
  temp = cv.lm(data.frame(bike_trn), model, m = kfold, 
               seed = 40, printit = FALSE, plotit = FALSE)
  if (length(temp) == 19){
    dat = cbind(temp[2], temp[17]^2, temp[18]^2, temp[19])
  }
  else if (length(temp) == 18){
    dat = cbind(temp[2],temp[16], temp[17], temp[18])
  }
...
plot_CV = ggplot(res, aes(x = MAE, y = MSE,  color = model)) +
  geom_point(size = 2.5) + theme_bw() + xlab("Mean Absolute Error") +
  ylab("Root Mean Squared Error") + ylim(0, 500) + xlim(0, 350) +
...
Next, we computed a large variety of metrics for the models to see if `selected_model` performed better in all regards compared to the `bic_sqrt` and `bic_model` models, which it  did.

```{r, warning = FALSE, message = FALSE}
# Code for Table 3.4
Actual_Test = bike_tst$RentedBikeCount
metrics = sapply(list(predict(model_add, bike_tst), predict(bic_model, bike_tst),
                      predict(bic_sqrt, bike_tst)^2, predict(selected_model, bike_tst)^2), function(x){
...
metrics <- mapply(  # Iterate over each column
  function(df, digits) { formatC(df, format="f", digits=digits) },
  df=metrics, digits=c(1, 1, 1, 1, 3, 3, 3)
)
metrics <- data.frame(t(metrics))  # Transpose back
...
table_metrics = kable(metrics,  
                        caption = "Evaluation Metrics of Test Set of Models") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered", "condensed", "responsive"))
```

We set up several plots and models below to see how well the model worked and produced accurate predictions, as well as to analyze, discover and adjust the model to any abnormal behavior.

```{r, warning = FALSE, message = FALSE}
#Plot the actual bike count vs predicted and error in predictions (Figure 3.6 and Figure 3.7)
data$Predicted_BikeCount =  predict(selected_model, data)^2
...
```{r, warning = FALSE}
#Plot timeline of actual bikes rented vs predicted (Figure 3.8 and Figure 3.9)
Predicted = predict(selected_model, data)^2
...
```{r, message = FALSE}
#Plot prediction with holiday analysis (Figure 3.10)
...
```{r, message = FALSE}
#Plot prediction with season analysis (Figure 3.11)
...
#Histogram of symmetric mean absolute percentage error for hours (Figure 3.12)
hourerror = data %>%
  group_by(Hour) %>%
...
```{r, message = FALSE, warning = FALSE}
#Histogram of symmetric mean absolute percentage error for months (Figure 3.13)
montherror = data %>%
  group_by(Month) %>%
  summarize(Error = 100 * Metrics::smape(RentedBikeCount, Predicted_BikeCount))
...
Table 3.1 below shows how the initial models performed on the Test set. The larger models will almost always have a larger $R^{2}$ value and this is evident from the table. The 'All Interaction with Quadratics' model with 310 coefficients has the highest adjusted $R^{2}$. We chose the 'All Interaction (Quadratic for Wind/Temp)' model with 214 parameters as the best initial model as it is a less complex model than the largest model and it performed well on all the evaluation metrics on the Test set.

```{r tab1, echo = FALSE, fig.align = 'center'}
table_eval1
```

Figure \@ref(fig:fig1) shows the diagnostics for the best model (BIC model with 137 parameters) chosen by running the 'All Interaction (Quadratic for Wind/Temp)' model with 214 parameters through backward AIC and BIC procedures. The Fitted Vs Residual plot shows that the constant variance assumption is violated since there is more variance of the residuals as the fitted values increase. The Q-Q plot of the residuals also shows fat tails meaning there is more data located at the extremes compared to a normal distribution. We wanted to check if a transformation of the response variable can fix this. 
...
From the Table 3.2 below it is evident that the square root transformation performed the best on the test set with adjusted $R^{2}$ of 0.902.

```{r tab2, echo = FALSE, fig.align = 'center'}
table_transform
```

The Q-Q plot and Fitted Vs residual plot clearly showed the normalcy and equal variance violations. So, the idea is to transform the response variable, so that the regression residuals become normally-distributed. The optimal lambda value from the Box-Cox power transform is $\lambda \approx 0.3$ but we achieved the lowest RMSE on the test set and a highest adjusted $R^{2}$  with a $\lambda = 0.5$ which is a square root transformation. Also with the square root transformation, we do not have the issue of NaN's popping up when transforming the response variable back since a negative number squared is a real number. This was an issue with some of the other $\lambda$s.

```{r fig2, fig.cap = "Box-Cox Plot from BIC Model with no Transform", echo = FALSE, fig.align = 'center'}
plot_boxcox
```

The Fitted Vs Residuals plot in Figure \@ref(fig:fig3) for the Selected Model with square root transform of the response variable shows a large improvement in terms of obtaining constant variance compared to the BIC Model without transformation. However, there is no improvement in the Q-Q plot of the residuals as it still shows fat tails compared to a normal distribution.

```{r fig3, fig.cap = "Fitted vs Residuals and Q-Q Plot for Selected Model with Sqrt Transform", echo = FALSE, fig.align = 'center'}
grid.draw(plot_diag2)
```

Table 3.3 below shows the error metrics on the Test set for different models. The BIC of the best model chosen previously greatly reduced after applying a square root transform of the response variable. The square root transform model with manual selection, lets call this the "Selected Model (Sqrt Transform)", of predictors emerged as the best model with the least error LOOCV-RMSE and least BIC.
...
Figure \@ref(fig:fig7) shows that all the points are closer to the regressed diagonal line and follow the same trend as of the regression line giving us confidence in our predictions.
...
Figure \@ref(fig:fig6) shows the Actuals Vs Errors plotted. Most of the points are densely packed in the lower error regions around 0.

```{r fig6, fig.cap = "Predicted vs Actual Number of Rented Bikes of Test Set using Selected Model (Sqrt Transform)", warning = FALSE, message = FALSE, fig.cap = "Actual Number of Rented Bikes vs Error of Test Set using Selected Model (Sqrt Transform)", echo = FALSE, fig.align = 'center'}
plot_error
```

Figure \@ref(fig:fig8) through Figure \@ref(fig:fig11) shows the Predicted Vs Actual with respect to different predictor Variables. In all the plots the predicted line captures the general pattern of the Actuals and is very close to the Actual line indicating a good fit.
...
Table 3.1 displays the results we received when running the initial models for the data.  Starting with a purely additive multiple linear regression model, then adding complexity by including 'Hour' as a factor, interactions with 'Temp' and 'Humidity', and a full two-way interaction model on all predictors within the dataset.  The additive model, and subsequent models with limited interactions, performed poorly against the metrics established.  Using an 'All Interaction' model, which contained all two-way predictor interactions, continued to improve the LOOCV-RMSE, Test RMSE, BIC and $R^2$ metrics at the cost of model complexity, almost doubling the number of coefficients being used. After some trial and error testing, we found the 'All Interaction' with quadratics for 'Wind' and 'Temp' provided even better results. A full interaction model with quadratics with 333 coefficients had a higher adjusted $R^{2}$ but worse LOOCV-RMSE, Test RMSE, and Test $R^{2}$, indicating this model is overfit.

Using the 'All interaction' model with quadratics for Wind/Temp as a new base for the "best so far" model, we refined this model using a AIC and BIC stepwise procedure. The BIC model performed just as well as the AIC model and 'All interaction' model with quadratics for Wind/Temp in our test metrics according to Table 3.3, but with a substantial decrease in the number of coefficients from 161 to 137 and 214 to 137. For this reason, we chose the BIC model for further testing. 

Figure \@ref(fig:fig1) illustrates the results of performing a "Fitted vs Residuals" and "Normal Q-Q" plot for the BIC model. The "Fitted vs Residuals" plot shows the constant variance assumption is violated since the variance increases as the fitted values increase. The normality of the residuals is also suspect, given the fat tails seen in the Q-Q plot. To alleviate the violation of the constant variance assumption, we tried power transformations on the response variable and also calculated the test RMSE, as shown in Table 3.2. The optimal lambda value from the Box-Cox power transform is $\lambda \approx 0.3$ but we achieved the lowest RMSE on the test set and the highest adjusted $R^{2}$  with a $\lambda = 0.5$ which is a square root transformation. Also with the square root transformation, we do not have the issue of NaN's popping up when transforming the predicted value back to the original response variable since a negative number squared is a real number. This was an issue with some of the other $\lambda$s. Therefore, the square transformation was the preferred one.

Figure \@ref(fig:fig3) illustrates the effects on the "Fitted vs Residuals" and "Normal Q-Q" after the square root transformation on the response. Although the constant variance assumption from the Fitted vs Residuals plot improved substantially by the transformation, Q-Q Plot still shows fat tails, which means the residuals are more spread out than expected from a normal distribution.

To find the final model,  we used a stepwise BIC procedure from the full interaction with quadratics model (Square Root the response this time) and manually adjusted the model through trial and error.  We found the 'Selected Model (Sqrt Transformation)' that, although increased the number of coefficients, provided the best results for the LOOCV-RMSE, BIC, Test RMSE and Test $R^2$. Table 3.3 shows the evaluation metrics of the AIC Model (No Transform), BIC Model (No Transform), BIC Model (Transform), and the Selected Model. 

Once we found our chosen model, we ran a 10-fold cross validation analysis on it, as well as other tested models for comparison. The plot of the 10-fold Cross-Validation results on Figure \@ref(fig:fig4) shows the selected model performs better than the BIC model with both generally lower RMSE and MAE. Also, a comparison of the selected model to the additive model is shown and the selected model performs far better than the additive model. 

Figure \@ref(fig:fig5) does not suggest significant over fitting either as the variance of the selected model (measured by Mean Absolute Difference between the Cross-Validation Prediction and the Prediction from the model using the entire training set) does not appear to be higher than the BIC model.

Table 3.4 shows the more evaluation metrics on the test set of the additive model, BIC model (no transform), BIC model (transform), and the final model. The Selected Model performs better than the BIC model on all the metrics. 
An interesting note is how the square root transforming the response variable dramatically reduced the median error and percentage error but a smaller reduction in MSE. 
The baseline additive model is included as reference, which shows how much the selected model improves on the baseline. The Test RMSE, Test Mean Absolute Error, and Test $R^{2}$ were 188.1, 116.9, and 0.910 for the Selected Model respectively.  The Test RMSE, Test Mean Absolute Error, and Test $R^{2}$ were 418.0, 318.7, and 0.556 for the Additive Model respectively. 

We also looked at our predicted rented bike count vs actual rented bike count from the test data in Figure \@ref(fig:fig7).
While clearly not perfect, this figure shows the predictions follow a similar pattern of the regression line, with predictions starting to overestimate when the bike count grows larger (1500+). 
This error is also evidenced in Figure \@ref(fig:fig6). Although most points center around zero, particularly at the lower bike rental rates, the model begins to over predict rentals once we have less data at the higher bike rental points.

In Figures \@ref(fig:fig8) and \@ref(fig:fig9), we selected two timelines of 50 consecutive hours to test the predicted bike rentals vs actual rentals as a time series. It can be seen that the prediction generally follows the actual trend of rented bikes, though does not match exactly. 

Figure \@ref(fig:fig10) shows the mean actual vs predicted bike rentals by hour of the day separated by holidays and non-holidays. We can see there is a bias in the model on holidays, as there is a spike in predictions in the morning commute hours that is not there in reality. We see a similar problem for the evening commute hours. On average, the mean prediction is quite close to the actual mean observations, with the exception of days on a holiday as stated previously. Figure \@ref(fig:fig11) does the same analysis for season (rather than holiday).  This analysis also shows the average prediction by hour follows the actual observations quite closely.  Figure \@ref(fig:fig12) and \@ref(fig:fig13) also plots our symmetric mean error by hour and month.  We can see that the error slowly rises in the early hours of the day, peaking around the 4 hour mark. September has the highest error of any give month, while the summer months tend to have a lower error.

Based on our results discussed above, the model we've chosen seem to work somewhat well to predict the number of bikes rented at any given hour on any given day. Figures \@ref(fig:fig7) and \@ref(fig:fig6) show that, although the average trend is accurate, the number of bikes rented at any given hour can be significantly off based on what was predicted. However, as mentioned before, Figures \@ref(fig:fig8) and \@ref(fig:fig9) suggest the model is good when looking at the data as a time series. A model such as the one we found would help a company predict on average how many bikes should be on hand/available since the errors are usually not very large. For example, a positive offset can be applied to the predicted value so that there is not a shortage of bikes but yet not a large surplus of bikes.


# Appendix

Below is code used in the analysis, but cluttered the report plus the authors of the project.  The code is re-printed here, but not evaluated, to show what was used to create, evaluate and select the model. This is primarily the code in the 'Results' section for various graphs/tables to illustrate how well the model runs in comparison to the actual data in the 'Test' dataset.
...
The authors of this project are Scott Downey, Sushma Ponna, and William Shih.
</div>